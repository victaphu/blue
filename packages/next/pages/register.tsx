...
const useContractWriter = (args: any, functionName: string, address: string, abi: any) => {
    ...
    const { data, error, isError, write } = useContractWrite(config as any)
...
const Home: NextPage = () => {
    ...
    const getIcon = () => {
        if (steps > 0 && steps < 4) {
            return (<svg className="animate-spin -ml-1 mr-3 h-4 w-4 text-black" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        <div className="flex flex-col min-h-full items-center justify-center px-4 sm:px-6 lg:px-8">
            ...
            <div className="w-full max-w-md space-y-4 text-center">
                ...
                <div className="text-2xl">
                    <button className="btn btn-success w-full" onClick={generateNft} disabled={steps > 0 && steps < 4}><span className='mr-2'>{getIcon()}</span> {getTitle()}</button>
                    <p className='color-base-300 text-xs mt-2'><b>To begin lets create a free NFT! </b>We will prompt you to sign 3 signatures to redeem your free Fairy Token. Click Start Registration to begin</p>
                </div>
