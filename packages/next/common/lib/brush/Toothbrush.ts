...
export class Toothbrush {
  ...
  public static async isToothbrush (server: any): Promise<boolean> {
    try {
      const services = await server.getPrimaryServices()
      const serviceSet = new Set(services.map((service:any) => service.uuid))
  ...
  public static async requestServer (): Promise<any> {
  ...
  private server: any
  private services: Map<string, Promise<any>>
  private characteristics: Map<string, any | null>
  ...
  private connectServices (): Map<string, Promise<any>> {
    const services = Toothbrush.services.map(
      (id: string): [string, Promise<any>] =>
  ...
  public async getCharacteristic (serviceId: string, id: string): Promise<any | null> {
    if (this.characteristics.has(id)) {
      return this.characteristics.get(id)!
    }
    const service = await this.services.get(serviceId) as any
    let result
    try {
